...
/// <c>sent</c> command, and that a transport failure is counted as <c>failed</c> and rethrown.
/// </summary>
public sealed class CommandServiceTests : IDisposable
{
    ...
    private CommandService CreateSut() =>
        new(_encoder, _publisher, _metrics, NullLogger<CommandService>.Instance);
