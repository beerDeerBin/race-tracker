...
describe('TrajectoryMap', () => {
    ...
    it('places the heading-oriented vehicle marker at the active point (y-flipped)', () => {
        const { container } = renderMap(points[1]!);

        const marker = container.querySelector('polygon')?.parentElement;
        // x=2,y=1 → translate(2 -1); heading π/2 → rotate(-90)
        expect(marker?.getAttribute('transform')).toBe('translate(2 -1) rotate(-90)');
    });
